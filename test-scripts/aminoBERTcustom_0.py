...
med_len = int(np.median([len(elem) for elem in classification_df['text']]))
...
tokenized_ds = ds.map(lambda d : tokenizer(d['text'], truncation=True, padding=True), batched=True)
...
training_args = TrainingArguments(
    output_dir=HOMEDIR+'/custom-model3',
    learning_rate=2e-4,
...
with open(HOMEDIR+'BERT-custom-test3.txt','w') as data: 
