...
for train_inds, test_inds in inds[split_no]:
    ...
    print('Tokenizing...')
    tokenizer = AutoTokenizer.from_pretrained('distilbert-base-uncased') #ESM tokenizer
    ...
    model = AutoModelForSequenceClassification.from_pretrained('distilbert-base-uncased') #standard pretrained bert

    training_args = TrainingArguments(
        output_dir='./models/custom-model-base_{}'.format(RUN),
        learning_rate=2e-3,
    ...
    with open('./results/BERT-custom-base-scores_{}.txt'.format(RUN),'w') as data: 
